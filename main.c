...
void TimerAInit()
{
    ...
    TIMER_A1->CTL &= ~0x3F7;//TACL for 0
    TIMER_A1->CTL |= 0x01D1;//, Upmode, Div 8, ACLK
    TIMER_A1->EX0 = 0;//1/1 div
    TIMER_A1->CCR[0] = 4096;
    TIMER_A1->CCTL[0] |= 0x10;
    //Dont start yet till delay funct
}
void TimerADelay(void)
{

        TIMER_A1->CCR[0] = 4096;
        TIMER_A1->CTL |= TACLR;

        while ((TIMER_A1->CCTL[0] & 1) == 0); //wait for delay
        TIMER_A1->CCTL[0] &=~ 0x01;
